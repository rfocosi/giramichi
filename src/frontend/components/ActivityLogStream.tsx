...
import { Activity, MoveRight, PlusCircle, Workflow as WorkflowIcon, Edit3 } from 'lucide-react';

interface ActivityLogStreamProps {
  logs: ActivityLog[];
}

export const ActivityLogStream: React.FC<ActivityLogStreamProps> = ({ logs }) => {
  ...
  return (
    <div className="glass-panel" style={{ padding: '16px', height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px', paddingBottom: '10px', borderBottom: '1px solid var(--border-glass)' }}>
        <Activity size={18} color="var(--accent-cyan)" />
        <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '0.95rem', fontWeight: 600, color: 'var(--text-main)' }}>
          AI Activity Stream
        </h3>
      </div>
